...
Deno.test(
  "✅ Happy Path - 'join-game' (Multiple Joiners)",
  {
  ...
  async (t) => {
    ...
    try {
      ...
      await t.step("Fail: Join with bad invite code", async () => {
        ...
        assertEquals(responseData.error, "Invalid or expired invite code");
      });
      ...
      await t.step("Fail: Join a game twice", async () => {
        ...
        const responseData = data as JoinGameResponse;
        assert(!responseData.success, "Function should have failed");
        assertEquals(responseData.error, "You are already in this game");
      });
      ...
      await t.step("Fail: Join a game that already started", async () => {
        ...
        const responseData = data as JoinGameResponse;
        assert(!responseData.success, "Function should have failed");
        assertEquals(responseData.error, "This game is not open for joining");
      });
    } finally {
      // --- 4. Teardown ---
