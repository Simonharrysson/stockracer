...
Deno.test("✅ Happy Path - 'start-game'", {
  ...
  try {
    ...
    await t.step("Execute: Host calls 'start-game'", async () => {
      const { data, error } = await hostClient.functions.invoke("start-game", {
        body: { game_id: gameId },
      });
    ...
    await t.step("Assert: Verify game state is 'DRAFTING'", async () => {
      ...
      assertEquals(game.round_categories?.length, 7, "Categories were not set");
    ...
    await t.step("Assert: Verify draft pools were created", async () => {
      const { count, error } = await adminClient.from("game_round_pools")
...
Deno.test("🛡️ Failure Case - 'start-game' (Auth & Logic)", {
  sanitizeResources: false,
  sanitizeOps: false,
}, async (t) => {
