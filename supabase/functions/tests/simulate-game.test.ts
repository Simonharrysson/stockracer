...
if (!SUPABASE_LOCAL_URL || !SERVICE_ROLE_LOCAL) {
  console.error("Env problems");
}

Deno.test(
  "🏁 Full Game Simulation: 2 players draft 7 rounds",
  {
    ...
    try {
      ...
      await t.step("Draft all rounds", async () => {
        ...
        while (iterations < maxIterations) {
          ...
          // Get taken symbols
          const { data: taken, error: takenErr } = await adminClient
            .from("game_picks")
            .select("symbol")
            .eq("game_id", gameId);
        ...
        const { data: game2 } = await adminClient
          .from("games")
          .select()
          .eq("id", gameId)
          .single();
        ...
        const { count: totalPicks } = await adminClient
          .from("game_picks")
          .select("*", { count: "exact", head: true })
          .eq("game_id", gameId);
        ...
        const { count: joinerCount } = await adminClient
          .from("game_picks")
          .select("*", { count: "exact", head: true })
          .eq("game_id", gameId)
